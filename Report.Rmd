...
b. EDA
For weather data, we choose daily average normal temperature, daily average wind speed, daily average snow fall and feel-like temperature as weather variables.

At first we plot average attendance through each season. 
```{r}
library(ggplot2)
library(dplyr)
#data
...
ggplot(df,mapping=aes(DAILYAverageWindSpeed,P))+
  geom_point(colour="royalblue3",shape=16)
ggplot(df,mapping=aes(df$DAILYSnowfall,P))+
  geom_point(colour="royalblue3",shape=16)
ggplot(df,mapping=aes(df$feel_like_temp,P))+
  geom_point(colour="royalblue3",shape=16)
```

From the plots above, we could see that whatever the weather situation is, the attendance could still be high or low, there seems no correlation between weathr situation and basketball attendance.
